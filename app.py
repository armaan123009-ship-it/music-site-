...
app = Flask(__name__, static_folder='dist', static_url_path='/_static')
...
def get_playlist_details(playlist_id):
    ...
    # Fallback for liked songs special playlist if needed, 
    # but library.astro handles it differently.
...
def format_song(item):
    ...
    try:
        ...
        if not video_id: return None # Skip non-playable items (playlists/albums) in song lists
...
def register():
    ...
    return send_from_directory(app.static_folder, 'register/index.html')
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not info or 'url' not in info:
                # Fallback search if direct watch fails
